...
<%@ page language="java" contentType="text/html; charset=Shift_JIS"
	pageEncoding="UTF-8"%>
<%
	Map<String, String> prefixMap = (Map<String, String>) getServletContext()
...
function update(){
	...
	$.post('/rdf_viewer/RDFUpdateServlet', 'selectType=' + selectType)
	.done(function( data ) {
    	createGraph(data.graph);
    	removePrefix()
    	removeFilter()
    	$('#prefixTbody').append(data.prefix);
    	$('#classTbody').append(data.classFilter);
    	$('#objectPropertyTbody').append(data.objectPropertyfilter);
	})
}
		...
		<div class="row h-100">
			<div class="col-2 border border-secondary fixed-top bg-white">
				<ul class="nav flex-column nav-pills">
					...
					<li>
						<form name="form1">
							<input type="radio" name="select_data" value="Tag" checked>Tag</input>
							<input type="radio" name="select_data" value="Scene">Scene</input>
						</form>
						<a class="nav-link" id="v-view-tab" data-toggle="pill"
							href="#v-view" role="tab" aria-controls="v-view"
							onclick="filterOK()" aria-selected="false">View</a> 
						<a class="nav-link" id="v-view-tab" data-toggle="pill" href="#v-view"
							role="tab" aria-controls="v-view" onclick="update()"
							aria-selected="false">Refresh</a>
					</li>
					<li>
						<a class="nav-link" href="#" data-toggle="modal"
						data-target="#prefixModal">PREFIX</a>
						<a class="nav-link" href="#"
						data-toggle="modal" data-target="#filterModal">FILTER</a>
					</li>
				</ul>
			</div>
			<div class="col-2"></div>
