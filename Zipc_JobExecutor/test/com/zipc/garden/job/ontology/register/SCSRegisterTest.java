...
public class SCSRegisterTest {
    ...
    @Test
    public void test01_RegistScenarioSetting() {
        VMFile fpsVMFile = getVMFile("TSD.fps");
        ...
        scsSetting.setUuid(file.getUuid());
        scsSetting.setSettingHash(file.getHash());
        scsRoot.getSettings().add(scsSetting);

        SCSPattern scsPattern1 = SCSFactory.eINSTANCE.createSCSPattern();
        scsPattern1.setId("1");
        scsSetting.getPatterns().add(scsPattern1);

        SCSPatternReference scsRef11 = SCSFactory.eINSTANCE.createSCSPatternReference();
        scsRef11.setFileId(fpsVMFile.getId());
        scsRef11.setPatternId(1);

        SCSPatternReference scsRef12 = SCSFactory.eINSTANCE.createSCSPatternReference();
        scsRef12.setFileId(bpsVMFile.getId());
        scsRef12.setPatternId(1);

        scsPattern1.getPatternreferences().add(scsRef11);
        scsPattern1.getPatternreferences().add(scsRef12);

        SCSPattern scsPattern2 = SCSFactory.eINSTANCE.createSCSPattern();
        scsPattern2.setId("2");
        scsSetting.getPatterns().add(scsPattern2);

        SCSPatternReference scsRef21 = SCSFactory.eINSTANCE.createSCSPatternReference();
        scsRef21.setFileId(fpsVMFile.getId());
        scsRef21.setPatternId(1);

        SCSPatternReference scsRef22 = SCSFactory.eINSTANCE.createSCSPatternReference();
        scsRef22.setFileId(bpsVMFile.getId());
        scsRef22.setPatternId(2);

        scsPattern2.getPatternreferences().add(scsRef21);
        scsPattern2.getPatternreferences().add(scsRef22);

        IRDFRegister register = OntologyUtils.createRegister(scsRoot, file);
