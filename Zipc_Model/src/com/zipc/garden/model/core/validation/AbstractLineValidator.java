/**
 *
 * $Id$
 */
package com.zipc.garden.model.core.validation;

import com.zipc.garden.model.core.AbstractPoint;
import com.zipc.garden.model.core.LineType;
import org.eclipse.emf.common.util.EList;

/**
 * A sample validator interface for {@link com.zipc.garden.model.core.AbstractLine}. This doesn't really do anything, and it's
 * not a real EMF artifact. It was generated by the org.eclipse.emf.examples.generator.validator plug-in to illustrate how EMF's
 * code generator can be extended. This can be disabled with -vmargs -Dorg.eclipse.emf.examples.generator.validator=false.
 */
public interface AbstractLineValidator {
    boolean validate();

    boolean validateSourceAnchorX(double value);

    boolean validateSourceAnchorY(double value);

    boolean validateTargetAnchorX(double value);

    boolean validateTargetAnchorY(double value);

    boolean validateType(LineType value);

    boolean validateConnectionPoint(EList<AbstractPoint> value);

    boolean validateX(int value);

    boolean validateY(int value);
}
