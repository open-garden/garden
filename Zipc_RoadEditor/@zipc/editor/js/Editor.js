	...
	computeRoadMarkLineGeometry: function (lineObject) {
		if (lineObject !== undefined) {
			...
			if (lineObject.mode === 'none') {
				...
				const dashSize = mode.dashSize === Number.MAX_VALUE ? mode.dashSize : mode.dashSize * 6 / mode.precRate;
				const gapSize = mode.dashSize === Number.MAX_VALUE ? mode.dashSize : (mode.dashSize - mode.gapSize) * 6 / mode.precRate - 1;
				...
				for (let segment of lineObject.segments) {
					...
					if (lane !== undefined && lane !== null) {
						...
						geoMerged.push(lane.getRoadMarkLineExtrudeGeometry(parameters.width, parameters.offset, parameters.position, length, start, end, mode.precRate));
					}
				}

				let index = 0;
				for (let geo of geoMerged) {
					const vertices = geo.getAttribute('position');
					for (var l = vertices.count / 2, i = 0; i < l; i++) {
						if (index++ % dashSize > gapSize) continue;
						position.push(vertices.getX(i), vertices.getY(i), vertices.getZ(i));
					}
				}
				...
				for (let geo of geoMerged) {
					const vertices = geo.getAttribute('position');
					for (var l = vertices.count, i = l / 2; i < l; i++) {
						if (index++ % dashSize > gapSize) continue;
						position.push(vertices.getX(i), vertices.getY(i), vertices.getZ(i));
					}
				}
