...
var Loader = function (editor) {
	...
	this.loadFromJson = function (jsonData) {
		...
		// Route
		var egoCar = jsonData.egoCar;
		if (egoCar !== undefined && egoCar.routes.length > 0) {
			var egoCarRouteData = editor.computeRouteCurve4('EgoCar', '#' + new THREE.Color(0x0080ff).getHexString(), egoCar.routes[0]);
			egoCar.routeData = egoCarRouteData;
			var egoCarObject = new GARDEN.CarObject('EgoCar', 0x0080ff).setFromMetalData(egoCar);

			// editor.execute(new AddObjectCommand(editor, egoCarObject));
			new AddObjectCommand(editor, egoCarObject).execute();
		}

		if (jsonData.objects !== undefined) {
			for (var otherCar of jsonData.objects) {
				var color = Math.random() * 0xffffff;
				var otherCarRouteData = editor.computeRouteCurve4(otherCar.id, '#' + new THREE.Color(color).getHexString(), otherCar.routes[0]);
				otherCar.routeData = otherCarRouteData;
				var otherCarObject = new GARDEN.CarObject(otherCar.id, color).setFromMetalData(otherCar);

				// editor.execute(new AddObjectCommand(editor, otherCarObject));
				new AddObjectCommand(editor, otherCarObject).execute();
			}
		}
