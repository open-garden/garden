...
var MenubarFile = function (editor) {
	...
	option.onClick(function () {
		...
		try {
			...
			objects.filter(object => object.type === 'Car')
				.forEach(object => {
					...
					if (object.name.toUpperCase() === 'EGOCAR') {
						scenarioData['egoCar'] = metalData;
					} else {
						if (scenarioData['objects'] === undefined) scenarioData['objects'] = [];
						scenarioData['objects'].push(metalData);
					}
			...
			objects.filter(object => object.type === 'Trajectory')
				.forEach(object => {
					var metalData = JSON.parse(JSON.stringify(object.metalData));
					if (metalData.hasOwnProperty('objectId')) delete metalData['objectId'];
					if (metalData.hasOwnProperty('basePointsArray')) delete metalData['basePointsArray'];
					if (scenarioData['trajectories'] === undefined) scenarioData['trajectories'] = [];
					scenarioData.trajectories.push(metalData);
				});
			...
			for (let i = 0, l = objects.length; i < l; i++) {
				...
				if (object.type === 'Road') {
					...
					for (let child of object.children) {
						if (child.type === 'Lane' && child.metalData.type === 'driving') {
