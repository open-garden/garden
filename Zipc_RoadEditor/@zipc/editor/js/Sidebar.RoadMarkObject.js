...
var SidebarRoadMarkObject = function (editor) {
	...
	// Sign Skew
	var signSkewRow = new UIRow();
	var signSkew = new UIInteger(0).setWidth('50px').setRange(-180, 180).onChange(update);
	signSkewRow.add(new UIText(strings.getKey('sidebar/scenario/sign/skew')).setWidth('90px'));
	signSkewRow.add(signSkew);
	container.add(signSkewRow);
		...
		if (object !== null) {
			...
			if (object.type === 'RoadStructure') {
				...
				if (object.type === 'RoadMark.Line') {
					...
					if (parameters.skew !== signSkew.getValue()) {
						parameters.skew = signSkew.getValue()
					}
					if (parameters.mode === 'stop_line') {
	...
	function updateDrivingLaneOptions() {
		...
		(function addObjects(objects) {
			for (let i = 0, l = objects.length; i < l; i++) {
				const object = objects[i];
				if (object.type === 'Road') {
					const laneChildren = object.children.filter(child => child.type === 'Lane' && child.metalData.type === 'driving');
					for (let child of laneChildren) {
	...
	function updateUI(object) {
		...
		if (object.type === 'RoadStructure') {
			...
			signSkew.setValue(parameters.skew);
