...
LaneObject.prototype = Object.assign(Object.create(AbstractWay.prototype), {
    ...
    getRoadMarkLineExtrudeGeometry: function (position) {
        const linePosition = position === undefined ? 'right' : position;
        if (linePosition === 'right') {
            return this.curve02.clone();
        } else {
            return this.curve01.clone();
        }
    },
    ...
    getSilblingLaneSpriteInfo: function (silblingLaneIndex) {
        ...
        const siblingLane = this.parent.children.find(child =>
            ...
            && (child.metalData.type === 'driving' || child.metalData.type === 'ramp')
        );
    ...
    computeWaypoint: function () {
        ...
        const isRampLane = this.metalData.type === 'ramp';
        ...
        if (this.spriteEdge.offsetType === 'merge' && this.sprite02.position2.y < 0.5) {
            ...
            if (this.spriteEdge.offsetInward) {
                ...
                for (let i = 0, l = waypoints.length; i < l; i++) {
                    ...
                    if (isRampLane) {
                        this.extensionInfo[this.extensionInfo.length - 1].roadGeo.splice(0, 0, 'ramp');
                    }
                }
                return;
            } else {
    ...
    update: function () {
        ...
        this.refLine.visible = data.position !== 'center' && (data.type === 'driving' || data.type === 'ramp') ? this.isRefLineVisible() : false;
