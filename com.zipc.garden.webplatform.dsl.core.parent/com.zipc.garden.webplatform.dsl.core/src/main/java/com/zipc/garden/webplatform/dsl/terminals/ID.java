...
public class ID {
    ...
    // ('0'..'9')+
    private static final Pattern INT_PATTERN = Pattern.compile("-?(\\.\\d+)|(\\d+(\\.\\d*)?)");
    ...
    public enum Type {
        ...
        /**
         * A INT ID that complies with the INT terminal rule:
         * 
         * <pre>
        * terminal INT:
        * ('0'..'9')+;
        ...
        * ('"' ('\\' ('b' | 't' | 'n' | 'f' | 'r' | 'u' | '"' | "'" | '\\') | !('\\' | '"'))* '"');
        ...
        QUOTED_STRING,
        /**
         * A html string ID that complies to the HTML_STRING terminal rule (and its fragments):
         * 
         * <pre>
    ...
    /**
     * Constructs a new ID of the given type from the given (encoded) raw string value.
     * @param string The (encoded) raw string to create an ID for. Maybe <code>null</code>.
     * @param type The type of the ID.
     * @return A new ID, or <code>null</code> if the given raw string was <code>null</code>.
     */
    ...
    /**
     * Constructs a new ID from the given (encoded) raw value, inferring the 'best matching' type.
     * @param string The (encoded) raw string to create an ID for. Maybe <code>null</code>.
     * @return A new ID, or <code>null</code> if the given string was <code>null</code>.
     */
    public static ID fromString(String string) {
        ...
        if (INT_PATTERN.matcher(string).matches()) {
            return fromValue(string, Type.INT);
        }
        return fromString(string, Type.STRING);
    }

    /**
     * Constructs a new ID from the given (decoded) value, inferring the 'best matching' type.
     * @param value The (decoded) value to create an ID for. Maybe <code>null</code>.
     * @return A new ID, or <code>null</code> if the given value was <code>null</code>.
     */
    public static ID fromValue(String value) {
        ...
        if (INT_PATTERN.matcher(value).matches()) {
            return fromValue(value, Type.INT);
        }

        // check for STRING:
        if (STRING_PATTERN.matcher(value).matches()) {
            if (!value.equalsIgnoreCase("node") && !value.equalsIgnoreCase("edge") && !value.equalsIgnoreCase("graph") && !value.equalsIgnoreCase("digraph")
                    && !value.equalsIgnoreCase("subgraph") && !value.equalsIgnoreCase("strict")) {
    ...
    private static String decode(String string, Type type) {
        ...
        case QUOTED_STRING:
            return string
                    ...
                    /*
                     * 'As another aid for readability, dot allows double-quoted strings to span multiple physical lines using
                     * the standard C convention of a backslash immediately.'[footnote on
                     * http://www.graphviz.org/doc/info/lang.html]
                     */
                    .replaceAll("\\\\(?:\\r\\n|\\r|\\n)", "")
                    /*
                     * Un-escape escaped quotes: 'In quoted strings in DOT, the only escaped character is double-quote
                     * ("). That is, in quoted strings, the dyad \" is converted to " ; all other characters are left unchanged.
                     * In particular, \\ remains \\.' [footnote on http://www.graphviz.org/doc/info/lang.html]
                     */
                    .replaceAll("\\\\\"", "\"");//$NON-NLS-1$//$NON-NLS-2$
        case HTML_STRING:
