...
public class DSLCoreEObjectAtOffsetHelper {
    ...
    /**
     * @return the cross referenced EObject under, right or left to the cursor (in that order) or <code>null</code> if there is
     *         no cross referenced object next to the offset.
     */
     ...
     *         {@link CrossReference} <code>null</code> if the passed node was a null reference or if the passed node is not
    ...
    protected EObject resolveCrossReferencedElement(INode node) {
        ...
        if (referenceOwner != null) {
            ...
            if (crossReference == DSLCOREPackage.Literals.ABSTRACT_OPERATION__OPERATOR) {
